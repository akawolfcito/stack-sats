...
const props = defineProps<{
  ...
  /** Initial asset tab to display (defaults to 'stx') */
  initialAsset?: 'stx' | 'btc';
}>();
...
// Reset state when modal closes, set initial tab when opens
watch(
  () => props.visible,
  (visible) => {
    if (visible) {
      // Set initial tab based on prop when modal opens
      activeTab.value = props.initialAsset || 'stx';
    } else {
