...
const actionItems = computed<ActionItem[]>(() => {
  ...
  // Send is available for STX and BTC
  const canSend = isAvailable && (asset.value?.id === 'stx' || asset.value?.id === 'btc');
  return [
    { key: 'send', label: 'Send', variant: 'primary', disabled: !canSend },
...
function handleAction(key: string) {
  ...
  if (key === 'send') {
    // Navigate to appropriate send view based on asset
    if (asset.value?.id === 'btc') {
      router.push({ path: '/send-btc' });
    } else {
...
async function loadBtcBalance() {
  if (!currentAccount.value || asset.value?.id !== 'btc') return;

  // Get both BTC addresses (P2PKH + P2TR)
  const addresses: string[] = [];
  if (currentAccount.value.btcP2PKHAddress) addresses.push(currentAccount.value.btcP2PKHAddress);
  if (currentAccount.value.btcP2TRAddress) addresses.push(currentAccount.value.btcP2TRAddress);
...
onBeforeMount(async () => {
  ...
  // Load data for available assets
  if (asset.value?.id === 'stx') {
    await loadBalance();
    loadTransactions();
    loadTokens();
...
<template>
  <ScreenShell :padded="false" data-roi="asset-detail">
    ...
    <ReceiveModal
      ...
      :initial-asset="asset?.id === 'btc' ? 'btc' : 'stx'"
